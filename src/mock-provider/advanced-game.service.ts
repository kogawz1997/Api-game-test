...
export class AdvancedGameService {
  ...
  private async protectCallback(providerCode: string, actionName: string, body: AnyBody) {
    ...
    if (process.env.MOCK_CALLBACK_REQUIRE_IP_WHITELIST === 'true') {
      ...
      if (!clientIp || allowedIps.length === 0 || !allowedIps.map((ip) => this.normalizeIp(ip)).includes(clientIp)) {
        throw new UnauthorizedException({ success: false, code: 'IP_NOT_WHITELISTED', message: 'Callback IP is not whitelisted', data: { clientIp, allowedIps } });
      }
    }
    ...
    if (process.env.MOCK_CALLBACK_REQUIRE_NONCE === 'true') {
      ...
      try {
        await this.prisma.callbackNonce.create({ data: { providerCode, nonce, action: actionName, expiresAt: new Date(Date.now() + ttlSeconds * 1000) } });
      } catch (error: any) {
        if (error?.code === 'P2002') throw new UnauthorizedException({ success: false, code: 'REPLAY_DETECTED', message: 'Duplicate callback nonce rejected' });
        throw error;
      }
    }
  }
  ...
  private parseIpWhitelist(value?: string | null) {
    if (!value) return [];
    try { const parsed = JSON.parse(value); return Array.isArray(parsed) ? parsed.map(String) : [String(value)]; } catch { return value.split(',').map((item) => item.trim()).filter(Boolean); }
  }
  ...
  private async logApi(providerCode: string, action: string, request: unknown, response: unknown, status: string, startedAt = Date.now(), errorCode?: string) {
    await this.prisma.providerApiLog.create({ data: { providerCode, action, direction: action.startsWith('callback') ? 'inbound' : 'internal', requestMasked: JSON.stringify(this.maskSecrets(request)), responseMasked: JSON.stringify(this.maskSecrets(response)), status, durationMs: Date.now() - startedAt, errorCode } });
  }

  private requireString(value: unknown, field: string) {
    if (!value || typeof value !== 'string') throw new BadRequestException({ success: false, code: `MISSING_${field.toUpperCase()}`, message: `${field} is required` });
    return value.trim();
  }
  ...
  private maskSecrets(value: unknown): unknown {
    ...
    return Object.fromEntries(Object.entries(value as Record<string, unknown>).map(([key, raw]) => {
      ...
      if (lower.includes('secret') || lower.includes('key') || lower.includes('token') || lower.includes('password') || lower.startsWith('__')) return [key, '***masked***'];
