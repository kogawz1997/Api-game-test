...
export class AdvancedGameService {
  ...
  async createPlayer(body: AnyBody) {
    ...
    const player = await this.prisma.providerPlayer.upsert({
      ...
      update: {
        ...
        language: String(body.language || 'th'),
        status: String(body.status || 'active'),
        rawData: JSON.stringify(this.maskSecrets(body)),
      },
      create: {
        providerCode,
        memberId,
  ...
  async getPlayer(body: AnyBody) {
    const providerCode = this.requireString(body.providerCode, 'providerCode').toUpperCase();
    const memberId = this.requireString(body.memberId, 'memberId');
    const player = await this.prisma.providerPlayer.findUnique({
      where: { providerCode_memberId: { providerCode, memberId } },
    });

    if (!player) {
      throw new NotFoundException({ success: false, code: 'PLAYER_NOT_FOUND', message: 'Provider player not found' });
    }
  ...
  async disablePlayer(body: AnyBody) {
    const providerCode = this.requireString(body.providerCode, 'providerCode').toUpperCase();
    const memberId = this.requireString(body.memberId, 'memberId');
    const player = await this.prisma.providerPlayer.update({
      where: { providerCode_memberId: { providerCode, memberId } },
  ...
  async callbackBet(body: AnyBody) {
    return this.moneyCallback(body, 'callback_bet', 'bet', 'debit');
  }

  async callbackWin(body: AnyBody) {
    return this.moneyCallback(body, 'callback_win', 'win', 'credit');
  }
  ...
  async callbackSettle(body: AnyBody) {
    ...
    const round = await this.prisma.gameRound.upsert({
      ...
      update: {
        ...
        winAmount: new Prisma.Decimal(winAmount),
        validBetAmount: new Prisma.Decimal(validBetAmount),
        status: 'settled',
        settledAt: new Date(),
        rawData: JSON.stringify(this.maskSecrets(body)),
      },
  ...
  async callbackCancel(body: AnyBody) {
    return this.updateRoundStatus(body, 'cancelled', 'callback_cancel');
  }

  async callbackRollback(body: AnyBody) {
    return this.updateRoundStatus(body, 'rollback', 'callback_rollback');
  }
  ...
  async getRound(body: AnyBody) {
    ...
    if (!round) {
      throw new NotFoundException({ success: false, code: 'ROUND_NOT_FOUND', message: 'Game round not found' });
    }
  ...
  async getRounds(body: AnyBody) {
    ...
    const rounds = await this.prisma.gameRound.findMany({
      ...
      take: Math.min(Number(body.limit || 50), 200),
    });

    return this.ok(rounds.map((round) => ({ ...round, betAmount: Number(round.betAmount), winAmount: Number(round.winAmount), validBetAmount: Number(round.validBetAmount) })));
  }
  ...
  async getProviderApiLogs(body: AnyBody) {
    ...
    const logs = await this.prisma.providerApiLog.findMany({
      where,
      orderBy: { createdAt: 'desc' },
      take: Math.min(Number(body.limit || 50), 200),
    });
  ...
  async reportSummary(body: AnyBody) {
    ...
    const [transactions, rounds, wallets] = await Promise.all([
      ...
      this.prisma.providerWallet.findMany({ where: { ...(providerCode ? { providerCode } : {}), ...(memberId ? { memberId } : {}) } }),
    ]);
  ...
  async reconcile(body: AnyBody) {
    ...
    return this.ok({
      ...
      memberId,
      transactionCount: transactions.length,
      expectedBalance: Number(expected.toFixed(4)),
      actualBalance: actual,
      diff,
      status: Math.abs(diff) < 0.0001 ? 'matched' : 'mismatch',
    });
  }
  ...
  private async moneyCallback(body: AnyBody, actionName: string, txType: 'bet' | 'win', direction: 'debit' | 'credit') {
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      throw new BadRequestException({ success: false, code: 'INVALID_AMOUNT', message: 'amount must be greater than 0' });
    }

    const existed = await this.prisma.providerTransaction.findUnique({ where: { referenceId } });
    if (existed) {
      return this.ok({ duplicated: true, transaction: this.toTransaction(existed) });
    }

    const result = await this.prisma.$transaction(async (tx) => {
      const wallet = await tx.providerWallet.upsert({
        where: { memberId_providerCode: { memberId, providerCode } },
        update: {},
        create: { memberId, providerCode, balance: new Prisma.Decimal(0) },
      });
      ...
      if (afterBalance < 0) {
        throw new BadRequestException({ success: false, code: 'INSUFFICIENT_BALANCE', message: 'Wallet balance is not enough', data: { beforeBalance, amount } });
      }
      ...
      const round = await tx.gameRound.upsert({
        ...
        update: {
          memberId,
          gameCode,
          betAmount: txType === 'bet' ? { increment: amount } : undefined,
          winAmount: txType === 'win' ? { increment: amount } : undefined,
          validBetAmount: txType === 'bet' ? { increment: amount } : undefined,
          status: txType === 'bet' ? 'open' : 'paid',
          rawData: JSON.stringify(this.maskSecrets(body)),
        },
        create: {
          providerCode,
          ...
          gameCode,
          roundId,
          betAmount: new Prisma.Decimal(txType === 'bet' ? amount : 0),
          winAmount: new Prisma.Decimal(txType === 'win' ? amount : 0),
          validBetAmount: new Prisma.Decimal(txType === 'bet' ? amount : 0),
          status: txType === 'bet' ? 'open' : 'paid',
          rawData: JSON.stringify(this.maskSecrets(body)),
        },
      });
      ...
      const transaction = await tx.providerTransaction.create({
        data: {
          ...
          status: 'success',
          rawRequest: JSON.stringify(this.maskSecrets(body)),
          rawResponse: JSON.stringify({ afterBalance }),
        },
      });
  ...
  private async updateRoundStatus(body: AnyBody, status: string, actionName: string) {
    ...
    const round = await this.prisma.gameRound.update({
      where: { providerCode_roundId: { providerCode, roundId } },
      data: { status, rawData: JSON.stringify(this.maskSecrets(body)), settledAt: status === 'settled' ? new Date() : undefined },
    });
    await this.logApi(providerCode, actionName, body, { roundId, status }, 'success');
    return this.ok(this.toRound(round));
  }

  private async ensureProvider(providerCode: string) {
  ...
  private async logApi(providerCode: string, action: string, request: unknown, response: unknown, status: string, startedAt = Date.now(), errorCode?: string) {
    await this.prisma.providerApiLog.create({
      data: {
        providerCode,
        action,
        direction: action.startsWith('callback') ? 'inbound' : 'internal',
        requestMasked: JSON.stringify(this.maskSecrets(request)),
        responseMasked: JSON.stringify(this.maskSecrets(response)),
        status,
  ...
  private requireString(value: unknown, field: string) {
    if (!value || typeof value !== 'string') {
      throw new BadRequestException({ success: false, code: `MISSING_${field.toUpperCase()}`, message: `${field} is required` });
    }
    return value.trim();
  }
  ...
  private maskSecrets(value: unknown): unknown {
    ...
    return Object.fromEntries(
      Object.entries(value as Record<string, unknown>).map(([key, raw]) => {
        ...
        return [key, this.maskSecrets(raw)];
      }),
    );
  }

  private toTransaction(tx: any) {
    return { ...tx, amount: Number(tx.amount), beforeBalance: Number(tx.beforeBalance), afterBalance: Number(tx.afterBalance) };
  }

  private toRound(round: any) {
    return { ...round, betAmount: Number(round.betAmount), winAmount: Number(round.winAmount), validBetAmount: Number(round.validBetAmount) };
  }
